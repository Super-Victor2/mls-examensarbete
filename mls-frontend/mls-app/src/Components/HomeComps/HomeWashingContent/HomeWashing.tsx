...
function HomeBooking() {
    ...
    useEffect(() => {
        axios.get<washingApiResponse>("https://tm2znos4mf.execute-api.eu-north-1.amazonaws.com/menu")
            .then(response => {
                const filteredCards = response.data.data.filter(card => card.type === "Tvätt");
    ...
    return (
        <>
            <motion.section initial="offscreen" whileInView="onscreen" variants={variants} viewport={{ once: true, amount: 0.8 }}  className="washing__booking" id='washing__section'>
